...
The audience of this README is the **client developer**, i.e. someone who will **use** `hypermea-gateway`.

If you are an **API developer** who will make changes to `hypermea-gateway` itself, please see [`src/hypermea-gateway/README.md`](./src/hypermea-gateway/README.md).
...
`hypermea-gateway` provides URL templates for all collections by way of its `item` rel [[RFC6573](https://datatracker.ietf.org/doc/html/rfc6573)].
```json
{
    "_items": [
      ...
...
Here are the link relations provided by `hypermea-gateway`
...
* `hypermea-gateway` follows a field naming convention so you know when such a field is "embeddable"
